...
with open ('pairs_withconn.csv', 'w', encoding='utf-8', newline='') as f:
    ...
    csv_writer.writerow(['emotion_clause', 'cause_candidate', 'conn', 'pairs_len', 'correctness', 'is_cause_conn'])
...
for i in range(len(df)):
    ...
    text_len = df['pairs_len'][i].split(',')
    correctness = df['correctness'][i]
    # transform text_len
    for j in range(len(text_len)):
        text_len[j] = int(text_len[j])
    ...
    # Add conn
    pairs_len = str(text_len[0]) + ',' + str(text_len[1] + 1)
