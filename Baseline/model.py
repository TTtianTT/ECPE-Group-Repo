...
class Network(nn.Module):
    ...
    def forward(self, query, query_mask, query_seg, query_len, seq_len, emotion_pos, cause_pos, doc_len, adj, possibility_distribution, correctness, tokenized_sequence, q_type):
    ...
    def loss_pre_emo_cau(self, pred, true, mask, possibility_distribution):
        true = torch.FloatTensor(true.float()).to(DEVICE)  # shape: batch_size, seq_len
        mask = torch.BoolTensor(mask.bool()).to(DEVICE)
        pred = pred.masked_select(mask)
        true = true.masked_select(mask)
        # weight = torch.where(true > 0.5, 2, 1)
        # Need debug
        criterion = nn.BCELoss(weight=possibility_distribution)
        return criterion(pred, true)
...
class Pre_Predictions_emo_cau(nn.Module):
    ...
    def forward(self, doc_sents_h):
        ...
        # Concat dim = 0 TODO
        pred_emo_cau = torch.sigmoid(pred_e)
        # Concat dim = 1 TODO
