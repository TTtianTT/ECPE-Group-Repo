...
with open('pairs_withconn&possibility.csv', 'w', encoding='utf-8', newline='') as f:
    ...
    csv_writer.writerow(['emotion_clause', 'cause_candidate', 'conn_words', 'possibility_distribution',
                         'correctness', 'cause_conn_possibility', 'contrast_conn_possibility',
                         'conjunction_conn_possibility', 'condition_conn_possibility', 'restatement_conn_possibility'])
...
for i in range(len(df)):
    ...
    segments_ids = [0] * (len(tokenizer.tokenize(str(emotion_clause))) + 2) + \
                   [1] * (len(tokenizer.tokenize(str(cause_candidate))) + 2)
    ...
    cause_conn_possibility, contrast_conn_possibility, conjunction_conn_possibility, condition_conn_possibility, \
        restatement_conn_possibility = np.sum(possibility_distribution[0:4]), \
        np.sum(possibility_distribution[4:8]), np.sum(possibility_distribution[8:12]), \
        np.sum(possibility_distribution[12:16]), np.sum(possibility_distribution[16:20])

    # Write result in csv
    with open('pairs_withconn&possibility.csv', 'a', encoding='utf-8', newline='') as g:
        csv_writer = csv.writer(g)
        csv_writer.writerow([emotion_clause, cause_candidate, candidate_conn, possibility_distribution,
                             correctness, cause_conn_possibility, contrast_conn_possibility,
                             conjunction_conn_possibility, condition_conn_possibility, restatement_conn_possibility])
